...
Philote's unit system overlaps with that of OpenMDAO. The main difference is that the lack of units cannot be specified using the `None` type in Python. Instead it should be listed as `unitless`.

The currently defined units are listed here (taken from the OpenMDAO documentation, licensed under the Apache 2 license):
