...
- **`RemoteImplicitComponent`**: For implicit disciplines that solve residual equations $R(\text{inputs}, \text{outputs}) = 0$
...
Implicit components solve equations of the form $R(\text{inputs}, \text{outputs}) = 0$:
